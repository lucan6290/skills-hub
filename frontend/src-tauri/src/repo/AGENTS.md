...
解析社区仓库（`~/.skills-hub/skillshub/community-skills`）和自定义仓库（`~/.skills-hub/skillshub/custom-skills`）的路径，扫描仓库中的技能目录，将发现的技能注册到数据库，并移除已不存在的技能记录。

## 文件清单

| 文件 | 职责 |
|------|------|
| `community.rs` | 路径解析：`resolve_community_repo_path`（DB 优先 → `~/.skills-hub/skillshub/community-skills`）、`resolve_custom_repo_path`（DB 优先 → `~/.skills-hub/skillshub/custom-skills`）、`ensure_community_repo`（创建目录） |
...
```
community_repo_path:  DB setting → ~/.skills-hub/skillshub/community-skills
custom_repo_path:     DB setting → ~/.skills-hub/skillshub/custom-skills
```
